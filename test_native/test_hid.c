...
int test_layouts(void) {
    ...
    ESP_LOGI(TAG, "Logitech layout M705 passed");
    ...
    assert(0 == mouse_report.scroll);
    assert(-1 == mouse_report.tilt);
    mouse_report = parse_mouse_report(mouse1_reports[3], 8, &mouse_layout);
    assert(0 == mouse_report.buttons.val);
    assert(-2047 == mouse_report.x_displacement);
    assert(-2047 == mouse_report.y_displacement);
    assert(0 == mouse_report.scroll);
    assert(0 == mouse_report.tilt);
    mouse_report = parse_mouse_report(mouse1_reports[4], 8, &mouse_layout);
    assert(0 == mouse_report.buttons.val);
    assert(-1 == mouse_report.x_displacement);
    assert(-1 == mouse_report.y_displacement);
    assert(0 == mouse_report.scroll);
    assert(0 == mouse_report.tilt);
    mouse_report = parse_mouse_report(mouse1_reports[5], 8, &mouse_layout);
    assert(0 == mouse_report.buttons.val);
    assert(1 == mouse_report.x_displacement);
    assert(1 == mouse_report.y_displacement);
    assert(-127 == mouse_report.scroll);
    assert(127 == mouse_report.tilt);
    mouse_report = parse_mouse_report(mouse1_reports[6], 8, &mouse_layout);
    assert(0 == mouse_report.buttons.val);
    assert(2047 == mouse_report.x_displacement);
    assert(2047 == mouse_report.y_displacement);
    assert(127 == mouse_report.scroll);
    ...
    ESP_LOGI(TAG, "Logitech reports M705 passed");
    ...
    ESP_LOGI(TAG, "Trust Kuza layout passed");
    ...
    ESP_LOGI(TAG, "Trust Kuza reports passed");
    ...
    ESP_LOGI(TAG, "Fujitsu M520 layout passed");
    ...
    assert(0 == mouse_report.tilt);
    mouse_report = parse_mouse_report(mouse3_reports[2], 4, &mouse_layout);
    assert(1 == mouse_report.buttons.button1);
    assert(1 == mouse_report.buttons.button2);
    assert(1 == mouse_report.buttons.button3);
    assert(127 == mouse_report.x_displacement);
    assert(-127 == mouse_report.y_displacement);
    assert(0 == mouse_report.scroll);
    assert(0 == mouse_report.tilt);
    ...
    ESP_LOGI(TAG, "Stadia controller layout passed");
    ...
    pad_report = parse_gamepad_report(pad1_reports[2], 11, &pad_layout);
    assert(0 == pad_report.buttons.up);
    assert(0 == pad_report.buttons.down);
    assert(0 == pad_report.buttons.right);
    assert(0 == pad_report.buttons.left);
    assert(127 == pad_report.lx);
    assert(127 == pad_report.ly);
    assert(255 == pad_report.lt);
    assert(255 == pad_report.rt);

    pad_report = parse_gamepad_report(pad1_reports[3], 11, &pad_layout);
    assert(1 == pad_report.buttons.a);
    assert(1 == pad_report.buttons.b);
    assert(0 == pad_report.buttons.x);
    assert(0 == pad_report.buttons.y);
    assert(pad_report.lx == 64);
    assert(pad_report.ly == 192);
    assert(pad_report.rx == 32);
    assert(pad_report.ry == 224);
    assert(pad_report.lt == 128);
    assert(pad_report.rt == 128);

    pad_report = parse_gamepad_report(pad1_reports[4], 11, &pad_layout);
    assert(1 == pad_report.buttons.a);
    assert(1 == pad_report.buttons.b);
    assert(1 == pad_report.buttons.x);
    assert(1 == pad_report.buttons.y);
    assert(1 == pad_report.buttons.select);
    assert(1 == pad_report.buttons.start);
    assert(1 == pad_report.buttons.l1);
    assert(1 == pad_report.buttons.l2);
    assert(1 == pad_report.buttons.l3);
    assert(1 == pad_report.buttons.l4);
    assert(1 == pad_report.buttons.r1);
    assert(1 == pad_report.buttons.r2);
    assert(1 == pad_report.buttons.r3);
